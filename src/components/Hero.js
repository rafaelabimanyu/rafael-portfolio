...
export default function Hero() {
  ...
  return (
    <section
      id="hero"
      className="relative flex min-h-screen items-center justify-center px-6 pt-24 pb-20 lg:px-16"
    >
          ...
          >
            {stats.map((stat, i) => {
              ...
              return (
                <div
                  key={stat.label}
                  className={`stat-card ${style.cardClass} group flex cursor-default flex-col items-center gap-1 rounded-xl border border-zinc-800 bg-zinc-900/40 px-3 py-4 backdrop-blur-sm sm:gap-2 sm:px-4 sm:py-5`}
                >
                  <span
                    className={`font-heading text-2xl font-bold sm:text-3xl ${style.color} ${style.glowClass}`}
                  >
                    {stat.value}
                  </span>
